...
export const updateRecipe = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { name, category, ingredients, instructions } = req.body;
    const image = req.file?.path;
    const recipe = await recipes.findByIdAndUpdate(id, { name, category, ingredients, instructions, image }, { new: true });
    res.status(200).json({
      success: true,
      data: recipe,
    });
...
export const getRecsipesbyCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
